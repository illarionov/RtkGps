...
void *handle_connection(void *arg)
{
	...
	if (!con) {
		LOGWRITE(LOG_DEFAULT, "handle_connection: got NULL connection");
		thread_exit(0);
	}
	...
	if ((res = sock_read_lines(con->sock, line, BUFSIZE)) <= 0) {
		LOGWRITE(LOG_DEFAULT, "Socket error on connection %d", con->id);
...
get_connection (sock_t *sock)
{
	...
	if (!sin)
	{
		LOGWRITE (LOG_DEFAULT, "WARNING: Weird stuff in get_connection. nmalloc returned NULL sin");
		return NULL;
	}
...
reverse (const char *host)
{
  ...
  if (!host)
  {
	  LOGWRITE (LOG_DEFAULT, "ERROR: reverse() called with NULL host");
	  return NULL;
  }
