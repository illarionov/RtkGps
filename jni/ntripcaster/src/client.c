...
void client_login(connection_t *con, char *expr)
{
	...
	if (!con || !expr) {
		LOGWRITE(LOG_DEFAULT, "WARNING: client_login called with NULL pointer");
		return;
	}
...
*/
	...
	LOGWRITE(LOG_DEFAULT, "Accepted client %d [%s] from [%s] on mountpoint [%s]. %d clients connected", con->id,
...
{
	if (!client || !source) {
		LOGWRITE(LOG_DEFAULT, "WARNING: del_client() called with NULL pointer");
		return;
	}

	if (source && client->food.client && (client->food.client->virgin != 1) && (client->food.client->virgin != -1)) {
		if (source->num_clients == 0)
			LOGWRITE (LOG_DEFAULT, "WARNING: Bloody going below limits on client count!");
		else
			source->num_clients--;
	}
	util_decrease_total_clients ();
...
//	char *time;
	...
	if (!con) {
		LOGWRITE(LOG_DEFAULT, "WARNING: greet_client called with NULL pointer");
		return;
	}
...
//	sock_write_line (con->sock, "Date: %s %s", time, info.timezone);
	...
	LOGWRITE(ANDROID_LOG_INFO,"Sourcetable:%s",pathBuffer);
...
void parse_mount_authentication_file()
{
	...
	if (line[BUFSIZE-1] == '\0') {
		LOGWRITE(LOG_DEFAULT, "READ ERROR: too long authentication line in config file (exceeding BUFSIZE)");
	}
...
int add_authentication_mount(mount_t * mount)
{
	...
	if (out) {
		LOGWRITE(LOG_DEFAULT, "WARNING: Duplicate mount record %s, using latter", mount->name);
		nfree(out->name);
